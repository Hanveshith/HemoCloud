                ...
                <tbody>
                    {
                        data.map((e, i) =>
                            props.user == "bank" ? <tr className={status == "All" ? "" : status != e.status ? "hidden" : ""}>
                                ...
                                <td className='border p-3 cursor-pointer underline decoration-dotted underline-offset-4' onClick={() => setPopup(i)}>{e.Hospital}</td>
                                <td className='border p-3'><span className={(e.status == false ? "border-metal text-metal" : (e.status == true ? "border-yellowX text-yellowX " : (e.rejected == false ? "border-red text-red" : "border-green text-green"))) + ' border-2 px-4 py-2 rounded-xl hover:shadow-md'}>{e.status}</span></td>
                            </tr>
                        )
                    }
                </tbody>
